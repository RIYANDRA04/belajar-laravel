...
<head>
    ...
    <title>Create new account — ShoesAsia</title>
        ...
        <div class="w-full lg:w-[55%] p-8 lg:p-16 flex flex-col relative z-10 bg-card">
            ...
            <nav class="flex items-center justify-between mb-12">
                ...
                <div class="hidden sm:flex gap-8 text-sm font-semibold items-center">
                    <a href="{{ route('login') }}" class="text-gray-400 hover:text-white transition-colors duration-200">Masuk</a>
                    <span class="bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 to-purple-400 border-b-2 border-indigo-500/80 pb-0.5 select-none">Daftar</span>
                </div>
            </nav>

            <div class="max-w-md w-full mx-auto flex-1 flex flex-col justify-center">
                <p class="text-xs font-extrabold text-indigo-400 tracking-widest uppercase mb-3">MULAI GRATIS</p>
                <h1 class="text-4xl lg:text-5xl font-black mb-3 tracking-tight bg-clip-text text-transparent bg-gradient-to-br from-white via-slate-100 to-slate-400">Buat akun baru<span class="text-indigo-500">.</span></h1>
                <p class="text-gray-400 font-medium mb-10 text-sm">
                    Sudah punya akun? <a href="{{ route('login') }}" class="premium-link">Masuk</a>
                </p>
                ...
                <form method="POST" action="{{ route('register') }}" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="premium-input-wrapper rounded-2xl p-1 flex items-center relative overflow-hidden">
                            <input type="text" name="name" value="{{ old('name') }}" placeholder="First name" class="w-full bg-transparent text-white px-4 py-3 outline-none text-sm font-medium placeholder-gray-500" required>
                            <i data-lucide="contact-2" class="w-5 h-5 text-gray-500 mr-4 transition-all duration-300"></i>
                        </div>
                        <div class="premium-input-wrapper rounded-2xl p-1 flex items-center relative overflow-hidden hidden md:flex">
                            <input type="text" name="last_name" placeholder="Last name" class="w-full bg-transparent text-white px-4 py-3 outline-none text-sm font-medium placeholder-gray-500">
                            <i data-lucide="contact-2" class="w-5 h-5 text-gray-500 mr-4 transition-all duration-300"></i>
                        </div>
                    </div>

                    <div class="premium-input-wrapper rounded-2xl p-1 flex items-center relative overflow-hidden">
                        <input type="email" name="email" value="{{ old('email') }}" placeholder="Email" class="w-full bg-transparent text-white px-4 py-3.5 outline-none text-sm font-medium placeholder-gray-500" required>
                    ...
                    <div class="premium-input-wrapper rounded-2xl p-1 flex items-center relative overflow-hidden">
                        ...
                        <i data-lucide="settings-2" class="w-5 h-5 text-gray-500 mr-4 transition-all duration-300"></i>
                    </div>

                    <div class="premium-input-wrapper rounded-2xl p-1 flex items-center relative overflow-hidden">
                        <input type="password" name="password_confirmation" placeholder="Confirm Password" class="w-full bg-transparent text-white px-4 py-3.5 outline-none text-sm font-medium tracking-wide placeholder-gray-500" required>
                        <i data-lucide="shield-check" class="w-5 h-5 text-gray-500 mr-4 transition-all duration-300"></i>
                    </div>

                    <div class="flex flex-col sm:flex-row gap-4 pt-4">
                        <a href="{{ route('login') }}" class="btn-premium-secondary rounded-full py-4 px-8 text-xs font-extrabold text-center sm:flex-1 text-gray-300">Sudah punya akun?</a>
                        <button type="submit" class="btn-premium-shimmer rounded-full py-4 px-8 text-xs font-extrabold text-center sm:flex-1 text-white">Daftar Sekarang</button>
                    </div>
                </form>
            </div>
        </div>
